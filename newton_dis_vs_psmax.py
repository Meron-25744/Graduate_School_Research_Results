...
def f(x,y,z,i):
    return math.sqrt(estD(x,y,z,i))-d_false[i]
...
mu = 0                  # 平均
sigma_l = 2             # レーザー単体のブレの分散値　自分で設定 [cm^2]
sigma_d = 4             # ドローンが空中でぶれる時の分散値　自分で設定 [cm^2]
sigma = math.sqrt(a/num+sigma_l+sigma_d)               # 標準偏差 (cm^2)
...
d = np.arange(1,11,0.1)    # distance [m] 1~10[m] 表示は[m]で
d_2 = d*0.001           # distance [km] 計算は[km]で


trans_ef = [0 for i in range(dis_num)]      # レーザーの伝送効率
...
Eta_max = [0 for i in range(dis_num)]
S_lazer = 1    # レーザーの照射面積の割合
Alpha = (Sigma_1/Kappa)*pow((Kai/Lambda),Rho)

# 照射距離に対する最大給電効率,照射距離[m]
for i in range(dis_num):
    trans_ef[i] = (1/e)**(Alpha*d_2[i])
    Eta_max[i] = S_lazer*(Parcent_InsideCircle/100) * (trans_ef[i]*a2+b2/Ps)
    # print(Eta_max[i],d[i])
